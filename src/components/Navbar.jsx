...
function Navbar() {
  ...
  const message = encodeURIComponent(
    "Hi, I want to book marriage photography / videography from Vichu Graphy",
  );
    ...
    <nav className="bg-white/90 backdrop-blur-md shadow-sm sticky top-0 z-50 border-b border-sky-100">
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex justify-between items-center h-16">
          ...
          <Link
            to="/"
            className="flex items-center text-xl font-bold text-sky-700 tracking-wide hover:text-pink-500 transition"
            onClick={() => setIsOpen(false)}
          >
           <img src={logo} height="52" width="50" alt="Vichu Graphy Logo" />

            <span>Vichu Graphy</span>
          </Link>
          ...
          >
            <span
              className={`bg-sky-700 block transition-all duration-300 ease-out h-0.5 w-6 rounded-sm ${isOpen ? "rotate-45 translate-y-1" : "-translate-y-0.5"}`}
            ></span>
            <span
              className={`bg-sky-700 block transition-all duration-300 ease-out h-0.5 w-6 rounded-sm my-0.5 ${isOpen ? "opacity-0" : "opacity-100"}`}
            ></span>
            <span
              className={`bg-sky-700 block transition-all duration-300 ease-out h-0.5 w-6 rounded-sm ${isOpen ? "-rotate-45 -translate-y-1" : "translate-y-0.5"}`}
            ></span>
